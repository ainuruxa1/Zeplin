...
$sql = mysqli_query($db,"SELECT * FROM `posts` WHERE `tag1` = '$postTag' OR `tag2` = '$postTag' OR `tag3` = '$postTag' OR `tag4` = '$postTag' OR `tag5` = '$postTag'");
$sqlrow = mysqli_fetch_array($sql);

if(empty($sqlrow['id'])){
    echo json_encode(array(
        "result" => "error",
        "error" => "invalid tag or posts from tag not found (404)"
        ));
    exit();
}
...
$res = mysqli_query($db, "SELECT * FROM `posts` WHERE `tag1` = '$postTag' OR `tag2` = '$postTag' OR `tag3` = '$postTag' OR `tag4` = '$postTag' OR `tag5` = '$postTag' ORDER BY `date` DESC LIMIT $startIndex, $startFrom");
